...
local function CreateMainBar()
    ...
    dragFrame:SetScript("OnEnter", function(self)
        ...
        local elapsed   = GetTime() - loginTime
...
UpdateBar = function()
    ...
    if db.showXPPerHour or db.showLevelingTime then
        local elapsed   = GetTime() - loginTime
        local xpPerHour = elapsed > 60 and (sessionXPGained / elapsed * 3600) or 0
...
CreateOptionsPanel = function()
    ...
    local counter = optionsFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    counter:SetPoint("BOTTOMRIGHT", optionsFrame, "BOTTOMRIGHT", -16, 42)
    counter:SetText("2 / 2")
    counter:SetTextColor(.7, .7, .7)
...
evFrame:SetScript("OnEvent", function(self, event, arg1)
    ...
    elseif event == "PLAYER_LOGIN" then
        loginTime       = GetTime()
        sessionXPGained = 0
        if db then db.lastXP = UnitXP("player") end
        UpdateBar()
    ...
    elseif event == "PLAYER_XP_UPDATE" then
        if db then
            ...
            if delta > 0 then sessionXPGained = sessionXPGained + delta end
            db.lastXP = newXP
        end
        UpdateBar()
